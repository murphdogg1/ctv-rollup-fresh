-- Migration 005: Fix zero impressions with correct data types
-- This maintains the original bigint data types while filtering zero impressions
...
-- Update the app rollup view directly with correct data types
CREATE OR REPLACE VIEW rr_rollup_app AS
SELECT 
  campaign_id,
  content_network_name as app_name,
  SUM(total_impressions)::bigint as impressions,
  SUM(total_completes)::bigint as completes,
  CASE 
...
-- Update the genre rollup view directly with correct data types
...
-- Update the content rollup view directly with correct data types
